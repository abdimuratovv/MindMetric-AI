...
export default function App() {
  const { screen, goTo, openStudent, selectedStudentId, adminView, setAdminView, studentsView, setStudentsView, studentReturnScreen, user, onLoginSuccess, onProfileCompleted, logout, activeCall, enterCall, leaveCall } = useAppState();
  ...
  return (
    <PageBackground>
      ...
      {SHELL_SCREENS.includes(screen) && (
        <AppShell screen={screen} goTo={goTo} openStudent={openStudent} selectedStudentId={selectedStudentId} adminView={adminView} setAdminView={setAdminView} studentsView={studentsView} setStudentsView={setStudentsView} studentReturnScreen={studentReturnScreen} user={user} logout={logout} enterCall={enterCall} />
      )}
    </PageBackground>
  );
}
